...
const accCollapse = document.getElementById('accuplacer');
function satToAccCollapse() {
    return new bootstrap.Collapse(accCollapse,{toggle:false});
}

function dates() {
    // VARIABLE DECLARATIONS
    // multiple measures
    let dateOptionsMM = document.getElementById("mm-date");
    ...
    // accuplacer
    let dateOptionsAcc = document.getElementById("acc-date");
    ...
    dateOptionsAcc.addEventListener('input', function (evt) {
        switch (dateOptionsAcc.value) {
            ...
            case "4-5_Years_Ago":
                ...
                break;
            case "More_Than_5_Years_Ago":
                accHelpPopup.classList.remove("hidden");
                accHelpNotTakenPopup.classList.add("hidden");
                accHelp45Popup.classList.add("hidden");
                allAccInputs.forEach(inputBlock => {
                    inputBlock.classList.add("hidden");
    ...
    satToAccRedirect.addEventListener('input', function (evt) {
        switch (satToAccRedirect.value) {
            case "Not_Taken":
                satHelpPopup.classList.add("hidden");
                satHelpNotTakenPopup.classList.remove("hidden");
                satToAccCollapse();
                break;
            case "More_Than_5_Years_Ago":
                satHelpPopup.classList.remove("hidden");
